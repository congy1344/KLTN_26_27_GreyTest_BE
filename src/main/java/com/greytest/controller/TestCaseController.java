...
public class TestCaseController {
 ...
 @PostMapping("/api/projects/{projectId}/test-cases/generate") public org.springframework.http.ResponseEntity<GenerationJobAcceptedDto> generate(@PathVariable Long projectId,@RequestParam(required=false) Long planId,@RequestParam(required=false) String servicePath,@RequestParam(defaultValue="false") boolean resume,@RequestHeader("Authorization") String a){var actor=access(projectId,a);return org.springframework.http.ResponseEntity.accepted().body(jobs.submit(projectId,actor.getId(),GenerationProgressStage.TEST_CASE,()->{if(planId==null)service.generate(projectId,servicePath,resume);else service.regenerate(projectId,servicePath,planId);}));}
