...
public class TestCaseController {
 ...
 @PostMapping("/api/projects/{projectId}/test-cases/generate") public org.springframework.http.ResponseEntity<GenerationJobAcceptedDto> generate(@PathVariable Long projectId,@RequestParam(required=false) Long planId,@RequestHeader("Authorization") String a){var actor=access(projectId,a);return org.springframework.http.ResponseEntity.accepted().body(jobs.submit(projectId,actor.getId(),GenerationProgressStage.TEST_CASE,()->{if(planId==null)service.generate(projectId);else service.regenerate(projectId,planId);}));}
 @PostMapping("/api/projects/{projectId}/test-cases") @ResponseStatus(HttpStatus.CREATED) public TestCaseDto create(@PathVariable Long projectId,@RequestHeader("Authorization") String a,@Valid @RequestBody CreateTestCaseRequest r){access(projectId,a);return jobs.executeMutation(projectId,()->service.create(projectId,r));}
 @PostMapping("/api/projects/{projectId}/test-cases/approve") public List<TestCaseDto> approve(@PathVariable Long projectId,@RequestHeader("Authorization") String a){access(projectId,a);return jobs.executeMutation(projectId,()->service.approve(projectId));}
 @PutMapping("/api/test-cases/{caseId}") public TestCaseDto update(@PathVariable Long caseId,@RequestHeader("Authorization") String a,@Valid @RequestBody UpdateTestCaseRequest r){Long projectId=service.projectIdForCase(caseId);access(projectId,a);return jobs.executeMutation(projectId,()->service.update(caseId,r));}
