...
public class TestCaseController {
 ...
 @PostMapping("/api/projects/{projectId}/test-cases/generate") public org.springframework.http.ResponseEntity<GenerationJobAcceptedDto> generate(@PathVariable Long projectId,@RequestParam(required=false) Long planId,@RequestHeader("Authorization") String a){access(projectId,a);return org.springframework.http.ResponseEntity.accepted().body(jobs.submit(projectId,GenerationProgressStage.TEST_CASE,()->{if(planId==null)service.generate(projectId);else service.regenerate(projectId,planId);}));}
